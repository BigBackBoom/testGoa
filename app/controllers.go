...
func MountActionController(service *goa.Service, ctrl ActionController) {
	initService(service)
	var h goa.Handler
	service.Mux.Handle("OPTIONS", "/api/v1/action/ping", ctrl.MuxHandler("preflight", handleActionOrigin(cors.HandlePreflight()), nil))
	...
	h = func(ctx context.Context, rw http.ResponseWriter, req *http.Request) error {
		...
		// Build the context
		rctx, err := NewPingActionContext(ctx, req, service)
		if err != nil {
			return err
		}
		return ctrl.Ping(rctx)
	}
	h = handleActionOrigin(h)
	service.Mux.Handle("GET", "/api/v1/action/ping", ctrl.MuxHandler("ping", h, nil))
	service.LogInfo("mount", "ctrl", "Action", "action", "Ping", "route", "GET /api/v1/action/ping")
}

// handleActionOrigin applies the CORS response headers corresponding to the origin.
func handleActionOrigin(h goa.Handler) goa.Handler {
